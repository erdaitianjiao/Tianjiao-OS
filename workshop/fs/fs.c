...
int32_t sys_mkdir(const char* pathname) {
    ...
    uint8_t rollback_step = 0;      // 用于操作失败时回滚状态
    ...
    if (inode_no != -1) {
        ...
        uint32_t pathname_depth = path_depth_cnt((char*)pathname);
        uint32_t path_searched_depth = path_depth_cnt(searched_record.searched_path);
        // 先判断是否把pathname的各层目录都访问到了 即是否再某个中间目录就失败了
        if (pathname_depth != path_searched_depth) {

            // 说明并没有访问到全部路径 某个中间目录是不存在的
    ...
    struct dir* parent_dir = searched_record.parent_dir;
    // 目录名称可能有字符 '/', 所以最好直接用searched_record.searched_path
    ...
    if (block_lba == -1) {
        ...
        printk("sys_malloc: block_bitmap_alloc for create directory failed\n");
        rollback_step = 2;
        goto rollback;
...
// 在inode编号为p_inode_nr的目录中查找 inode编号为 c_inode_nr子目录的名字
// 将名字写入缓冲区path 成功返回0 失败返回-1
static int get_child_dir_name(uint32_t p_inode_nr, uint32_t c_inode_nr, char* path, void* io_buf) {

    struct inode* parent_dir_inode = inode_open(cur_part, p_inode_nr);
    // 填充all_blocks 将该目录所占的扇区全部写入all_blcoks
    uint8_t block_idx = 0;
...
// 把当前工作目录绝对路径写入buf size是buf大小 当buf为NULL时 由操作系统分配存储工作目录的空间并返回地址
char* sys_getcwd(char* buf, uint32_t size) {
    ...
    // 即child_inode_nr为根目录inode编号0时停止
    ...
    // 至此full_path_reverse中的路径是反的 即子目录在左 父目录在右
    char* last_slash;
